...
- Name: fr13-dashboard.spec.js >> FR-13 Dashboard (admin) — Run by: 23127325 >> TC-FR13-010 — BVA: adding a 30,000,000 VND delivered order increases the revenue by 60,000,000
- Location: scripts\fr13-dashboard.spec.js:191:5

# Error details

```
Error: revenue delta must equal 60000000 (= amount × 2 × N)
  ...
  - generic [ref=f1e15]:
    ...
    - generic [ref=f1e17]:
      - generic [ref=f1e18]:
        ...
        - paragraph [ref=f1e20]: 62,400,002 ₫
      - generic [ref=f1e21]:
        - heading "Tổng số đơn hàng" [level=3] [ref=f1e22]
        - paragraph [ref=f1e23]: "60"
```
...
> 324 |           ).toBe(tc.expectedDelta);
  ...
  390 |           const displayedRevenue = await readRevenue(page);
  391 | 
  392 |           console.log(
  393 |             `[${tc.id}] correctRevenue=${correctRevenue} ` +
  394 |               `displayedRevenue=${displayedRevenue}`,
  395 |           );
  396 | 
  397 |           // Bug detector: displayed revenue must NOT equal correct.
  398 |           expect(
  399 |             displayedRevenue,
  400 |             `[BUG-FR-13-001] dashboard revenue (${displayedRevenue}) ` +
  401 |               `differs from the correct sum of delivered amounts ` +
  402 |               `(${correctRevenue}).`,
  403 |           ).not.toBe(correctRevenue);
  404 |           return;
  405 |         }
  406 | 
  407 |         default:
  408 |           throw new Error(`Unknown outcome "${tc.outcome}" in ${tc.id}`);
  409 |       }
  410 |     });
  411 |   }
  412 | });
